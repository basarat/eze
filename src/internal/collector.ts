import { RenderConfig, Data, SupportedMode, TableOfContentEntry, Watch } from '../types';
...
export class Collector {
  ...
  story({
    ...
    if (only) {
      if (this.only) {
        throw new Error(`We already have an only: "${this.only}" and you just passed in another: "${jsFileNameNoExt}"`);
      }
